...
This repo is a minimal Express HTTP server sandbox for AI agent POC testing. It exposes greeting, health check, health probe, version, user list, arithmetic addition, subtraction, multiplication, division, and exponentiation endpoints on port 3008 (configurable via `PORT`). The server can be run as a standalone process or imported as an Express app for integration testing in Node.
...
│   └── power.test.js      # Integration tests for POST /power (uses built-in fetch, no DB required)
...
- `test/power.test.js` — integration tests for `POST /power` using the exported app and Node's built-in `fetch`, mounted on an ephemeral port (no MongoDB required). Tests valid exponentiation (including zero exponent and zero base), missing operands, and validation errors.
- `package.json` — declares `"main": "src/index.js"` and the run scripts:
  ...
  - `npm test` → `node test/plus.test.js && node test/minus.test.js && node test/multiply.test.js && node test/divide.test.js && node test/power.test.js && …` (chained integration tests including arithmetic endpoints)
...
- **Testing**: Integration tests exist in `test/plus.test.js`, `test/minus.test.js`, `test/multiply.test.js`, `test/divide.test.js`, and `test/power.test.js`, run via `npm test`. Tests use Node's built-in `fetch`, ephemeral port, and no external test framework or DB.
...
- **Naming**: Routes are flat (`/hello`, `/health-check`, `/health`, `/version`, `/api/users`, `/plus`, `/minus`, `/multiply`, `/divide`, `/power`); single top-level `src/` directory with one file; tests in `test/` directory. Public endpoints have no auth; data comes from static in-memory store until a database is introduced.
...
- `POST /power` — accepts JSON body with numeric operands `{ "a": <number>, "b": <number> }` and returns HTTP `200` with JSON `{ "result": <a ** b> }`. Accepts integers, floats, negatives, and zero (e.g. any finite base with exponent 0 yields 1; base 0 with positive exponent yields 0). Rejects missing, null, non-numeric, `NaN`, or `Infinity` operands with HTTP `400` and JSON `{ "error": "a and b must be finite numbers" }`.
